...
const defaultState = {
  ...
  task1Enabled: true,
  task1Name: "제안양식 검토 및 사전작업",
  task1Start: "2026-04-01",
  task1End: "2026-04-03",
  task1Percent: 5,
  task1Workers: 1,
  statusText1: "제안양식 검토 및 사전작업",

  task2Enabled: true,
  task2Name: "대기",
  task2Start: "2026-04-04",
  task2End: "2026-04-20",
  task2Percent: 0,
  task2Workers: 4,
  statusText2: "대기",
  ...
  partner: "기술개발센터",
  detailText: `(4.1) 공고내용 및 기획팀 전달내용 확인 및 제안서 작성양식 검토
(4.6) 사전작업 필요부분 확인 (필요 템플릿 제작, 삽도가이드 설정  등)`
};
...
function renderVerticalGuides() {
  ...
  const visibleEnd = addDays(dates[dates.length - 1], 7);
  ...
  for (let i = 1; i < dates.length; i++) {
    ...
    const monthStartDate = new Date(curr.getFullYear(), curr.getMonth(), 1);

    if (monthStartDate < visibleStart || monthStartDate >= visibleEnd) continue;

    const line = document.createElement("div");
    line.className = "vertical-grid";
    line.style.left = `${xFromDate(monthStartDate)}px`;
...
function renderDetailLines() {
  ...
  lines.forEach((line, index) => {
    ...
    if (line.includes("(4.6)")) p.classList.add("is-orange");
...
function renderTexts() {
  ...
  planDateEndText.textContent = formatDisplayDate(state.plannedEnd, true);
